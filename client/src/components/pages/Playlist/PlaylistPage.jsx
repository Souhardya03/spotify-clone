...
const PlaylistPage = () => {
	const { id } = useParams();
	const { getSinglePlaylist, singlePlaylist, playSound, setCurrentSong } = useSpotify();
	...
	const handleClick = (index) => {
		...
		console.log(`Playing song index: ${index}, song: ${song.name}`);
	};
	...
	return (
		<div className="contentbackground lg:h-[90vh] h-[82vh] p-2 lg:p-4 overflow-auto rounded-md lg:ml-3 w-full">
			...
			<div className="text-white lg:flex lg:h-[30vh] items-center mt-8">
				...
				<div className="h-full w-full">
					<div className="flex flex-col gap-2 lg:gap-4 p-4 w-full">
						...
						<div className="lg:text-7xl text-5xl font-semibold">{singlePlaylist?.name}</div>
			...
			<div className="rounded-lg items-center lg:mt-4 lg:justify-start px-2 flex gap-6 w-full">
				<div className="w-14 flex justify-center items-center p-3 rounded-full bg-green-500" onClick={()=>handleClick(0)}>
					<img
			...
			<div className="rounded-lg mt-4 p-4 bg-[#252525] overflow-auto">
				...
				{singlePlaylist?.songs?.map((song, index) => (
					<div key={index} className="flex cursor-pointer justify-between hover:rounded-md duration-150 hover:bg-[#363535] px-2 py-3 text-white items-center gap-4 mt-4" onClick={() => handleClick(index)}>
						<div className="flex items-center gap-4">
							...
							<div className="flex gap-2 items-center">
								...
								<div className="text-white">
									...
									<div className="text-xs text-gray-400">{"by " + song?.artist?.firstName + song?.artist?.lastName}</div>
								</div>
							</div>
						</div>
